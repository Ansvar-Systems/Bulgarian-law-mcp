#!/usr/bin/env tsx
/**
 * Paid-tier database builder for Bulgarian Law MCP server.
 *
...
const DEFAULT_DB_PATH = path.resolve(__dirname, '../../../Bulgarian-law-mcp/data/database.db');
...
CREATE TABLE IF NOT EXISTS case_law (
  ...
  source TEXT NOT NULL DEFAULT 'vks_bg',
...
CREATE TABLE IF NOT EXISTS preparatory_works (
  ...
  committee TEXT,
  proposer TEXT,
  source TEXT NOT NULL DEFAULT 'parliament_bg',
  inserted_at TEXT DEFAULT (datetime('now'))
);
...
function buildPaidTier(): void {
  ...
  console.log('Building paid-tier extensions for Bulgarian Law MCP...\n');
  ...
  const updateMeta = db.transaction(() => {
    ...
    upsertMeta.run('jurisdiction', 'BG');
    upsertMeta.run('paid_tables', 'case_law,case_law_full,preparatory_works,preparatory_works_full,agency_guidance');
    upsertMeta.run('source_case_law', 'vks.bg,constcourt.bg');
    upsertMeta.run('source_prep_works', 'parliament.bg');
    upsertMeta.run('source_agency', 'cpdp.bg');
  });
  updateMeta();
