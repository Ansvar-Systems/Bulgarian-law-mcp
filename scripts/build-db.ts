#!/usr/bin/env tsx
/**
 * Database builder for Bulgarian Law MCP server.
 *
...
function buildDatabase(): void {
  console.log('Building Bulgarian Law MCP database...\n');
          ...
          if (extractedRefs.length > 0) {
            ...
            for (const ref of extractedRefs) {
              ...
              const euInsert = insertEuDocument.run(
                ...
                shortName, shortName, eurLexUrl, 'Auto-extracted from Bulgarian statute text',
              );
  ...
  const writeMeta = db.transaction(() => {
    ...
    insertMeta.run('jurisdiction', 'BG');
