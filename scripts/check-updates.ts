#!/usr/bin/env tsx
/**
 * Bulgarian Law MCP — Data Freshness Checker
 *
...
const PORTAL_URL = 'https://lex.bg';
const PORTAL_NAME = 'Lex.bg';
...
async function checkPortal(url: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': '@ansvar/bulgarian-law-mcp/1.0 (data-freshness-check)' },
    });
...
async function main(): Promise<void> {
  console.log('Bulgarian Law MCP — Data Freshness Check');
