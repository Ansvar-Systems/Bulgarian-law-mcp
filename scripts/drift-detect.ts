#!/usr/bin/env tsx
/**
 * Drift detection for Bulgarian Law MCP.
 *
 * Checks if upstream lex.bg content has changed since last ingestion.
...
async function main(): Promise<void> {
  console.log('Bulgarian Law MCP — Drift Detection');
  console.log('=====================================\n');
  ...
  for (const hash of fixture.provisions) {
    ...
    try {
      const response = await fetch(hash.upstream_url, {
        headers: { 'User-Agent': 'Bulgarian-Law-MCP/1.0 drift-detect' },
      });
