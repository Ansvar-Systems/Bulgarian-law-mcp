/**
 * HTML parser for Bulgarian legislation from the Sejm ELI API (api.sejm.gov.pl).
 *
 ...
 *
 * Bulgarian legislation references: Dz.U. YYYY poz. NNNN
 ...
 * to extract provisions from a Bulgarian statute.
 *
 ...
 *   </div>
 */
export function parseBulgarianHtml(html: string, act: ActIndexEntry): ParsedAct {
  ...
  for (let i = 0; i < articleStarts.length; i++) {
    ...
    // Bulgarian acts use "ilekroć mowa" (whenever mentioned), "rozumie się przez to"
...
/**
 * Extract definitions from Bulgarian legal text.
 *
 * Bulgarian definitions typically use patterns like:
