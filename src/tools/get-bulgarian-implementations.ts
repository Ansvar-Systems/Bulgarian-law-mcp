/**
 * get_bulgarian_implementations — Find Bulgarian statutes that reference a specific EU directive/regulation.
 */

import type Database from '@ansvar/mcp-sqlite';
import { generateResponseMetadata, type ToolResponse } from '../utils/metadata.js';

export interface GetBulgarianImplementationsInput {
  eu_document_id: string;
  primary_only?: boolean;
  in_force_only?: boolean;
}

export interface BulgarianImplementationResult {
  document_id: string;
...
export async function getBulgarianImplementations(
  db: InstanceType<typeof Database>,
  input: GetBulgarianImplementationsInput,
): Promise<ToolResponse<BulgarianImplementationResult[]>> {
  try {
  ...
  const rows = db.prepare(sql).all(...params) as BulgarianImplementationResult[];
