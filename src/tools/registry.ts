/**
 * Tool registry for Bulgarian Law MCP Server.
...
import { getBulgarianImplementations, type GetBulgarianImplementationsInput } from './get-bulgarian-implementations.js';
...
const LIST_SOURCES_TOOL: Tool = {
  ...
  description:
    ...
    'including the lex.bg (Bulgarian Government, Office of Parliamentary Counsel). ' +
    ...
    'Call this FIRST when you need to understand what Bulgarian legal data this server covers.',
  inputSchema: { type: 'object', properties: {} },
};
...
export const TOOLS: Tool[] = [
  {
    ...
    description:
      'Search Bulgarian statutes and regulations by keyword using full-text search (FTS5 with BM25 ranking). ' +
  ...
  },
  {
    ...
    description:
      'Build a comprehensive set of citations for a legal question by searching across all Bulgarian statutes simultaneously. ' +
      'Returns aggregated results from multiple relevant provisions, useful for legal research on a topic. ' +
      'Use this for broad legal questions like "What are the penalties for data breaches in Bulgaria?" ' +
  ...
  },
  {
    ...
    inputSchema: {
      type: 'object',
      properties: {
        document_id: { type: 'string', description: 'Bulgarian statute identifier.' },
        include_articles: {
          type: 'boolean',
  ...
  {
    name: 'get_bulgarian_implementations',
    description:
      'Find all Bulgarian statutes that reference or align with a specific EU directive or regulation. ' +
      'Given an EU document ID (e.g., "regulation:2016/679" for GDPR), returns matching Bulgarian statutes. ' +
      'Note: Bulgaria is an EU Member State and transposes EU directives into national law.',
    inputSchema: {
      type: 'object',
      properties: {
        eu_document_id: {
  ...
  },
  {
    ...
    description:
      'Search for EU directives and regulations that are referenced by Bulgarian legislation. ' +
    ...
    inputSchema: {
      ...
      properties: {
        ...
        has_bulgarian_implementation: {
          type: 'boolean',
          description: 'If true, only return EU documents referenced by Bulgarian legislation.',
        },
    ...
    try {
      ...
      switch (name) {
        ...
        case 'get_eu_basis':
          ...
          break;
        case 'get_bulgarian_implementations':
          result = await getBulgarianImplementations(db, args as unknown as GetBulgarianImplementationsInput);
          break;
