/**
 * search_eu_implementations — Search EU directives/regulations referenced by Bulgarian legislation.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_from?: number;
  year_to?: number;
  has_bulgarian_implementation?: boolean;
  limit?: number;
}
...
export interface EUImplementationSearchResult {
  ...
  bulgarian_statute_count: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      ed.title,
      ed.short_name,
      COUNT(DISTINCT er.document_id) as bulgarian_statute_count
  ...
  if (input.has_bulgarian_implementation) {
    sql += ' HAVING bulgarian_statute_count > 0';
  }
